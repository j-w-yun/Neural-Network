...
public class JayList<T>
{
	...
	*/
	@SuppressWarnings("unchecked") public synchronized boolean sort()
	{
		...
		// TODO O(n^2) to O(nlog(n))
		DifferentiableElement[] temp = new DifferentiableElement[size];
		DifferentiableElement hold;
		for(int j = 0; j < size; j++)
		{
			hold = (DifferentiableElement) get(j);
			DifferentiableElement placehold;
			for(int k = j; k < size; k++)
			{
				if(hold.getName().compareTo(((DifferentiableElement) get(k)).getName()) > 0)
				{
					placehold = hold;
					hold = (DifferentiableElement) get(k);
					remove(k);
					add((T) placehold, k);
				}
			}
